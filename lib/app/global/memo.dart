...
class MemoHelper {
  ...
  static Future<void> createTables(sql.Database database) async {
    ...
    await database.execute('''CREATE TABLE memo_test24(
  ...
  static Future<List<Map<String, dynamic>>> getItems() async {
    final db = await MemoHelper.db();
    return db.query('memo_test24', orderBy: "id");
  }

  static Future<List<Map<String, dynamic>>> getItem(int id) async {
    final db = await MemoHelper.db();
    return db.query('memo_test24', where: "id = ?", whereArgs: [id], limit: 1);
  }

  //get all createdAt
  static Future<List<Map<String, dynamic>>> getItemsCreatedAt() async {
    final db = await MemoHelper.db();
    return db.query('memo_test24', orderBy: "id", columns: ['id', 'createdAt']);
  }

  static Future<List<Map<String, dynamic>>> getItemsByDate(String date) async {
    final db = await MemoHelper.db();
    return db.query('memo_test24',
  ...
  static Future<List<Map<String, dynamic>>> getItemsByContent(
    ...
    final db = await MemoHelper.db();
    return db.rawQuery(
        "SELECT * FROM memo_test24 WHERE content LIKE '%${content}%'");
  }
